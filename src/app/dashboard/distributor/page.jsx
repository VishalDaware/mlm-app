...
import { getProducts, getDownline, addUser, createSale, getSalesForUser } from '@/services/apiService';
...
export default function DistributorDashboard() {
  ...
  const fetchData = useCallback(async () => {
    if (user) {
      try {
        const [productData, downlineData, salesData] = await Promise.all([
          getProducts(),
  ...
  useEffect(() => {
    if (user && sales.length > 0) {
      ...
      setAnalytics({ earnings: totalEarnings, teamSize: downline.length });
        ...
        setAnalytics({ earnings: 0, teamSize: downline.length });
    }
  }, [sales, downline, user]);
