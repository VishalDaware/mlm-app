...
// 1. Import addUser
import { getProducts, createSale, getSalesForUser, getHierarchy, addUser } from '@/services/apiService';
...
export default function DealerDashboard() {
  ...
  const [newFarmerName, setNewFarmerName] = useState(''); // 2. Add state for new farmer's name

  const fetchData = useCallback(async () => {
    if (user) {
      try {
        const [productData, salesData, hierarchyData] = await Promise.all([
          getProducts(),
  ...
  // 3. Add handler function to create a new farmer
  const handleAddFarmer = async (e) => {
    ...
    try {
      ...
      setNewFarmerName('');
      toast.success(`Farmer "${newFarmerName}" added successfully!`);
      fetchData(); // Refresh all data to show the new farmer in the hierarchy
    } catch (error) {
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <main className="container mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 flex flex-col gap-8">
          {/* Earnings, Sales History, and Hierarchy sections remain the same */}
          <div className="bg-white p-6 rounded-lg shadow-lg text-center"><h3 className="text-stone-500 text-sm font-semibold tracking-wider uppercase">Total Earnings</h3><p className="text-4xl font-bold text-green-600 mt-2">₹{totalEarnings.toFixed(2)}</p></div>
        ...
        {/* The right sidebar now includes the "Add Farmer" section */}
