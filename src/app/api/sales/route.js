...
export async function POST(request) {
  try {
    ...
    const { buyerId, productId, quantity } = await request.json();
    const sellerId = loggedInUser.id;

    const [seller, buyer, product, sellerInventory] = await Promise.all([
    ...
    const newTransaction = await prisma.$transaction(async (tx) => {
      ...
      const transaction = await tx.transaction.create({
        data: {
          sellerId, buyerId, productId, quantity, purchasePrice, totalAmount, profit,
        },
      });
